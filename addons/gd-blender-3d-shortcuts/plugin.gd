...
func _select_all_nodes(): #WORKING by forgeworksdev - Modified plugin
	...
	var all_nodes = Utils.recursive_get_children(scene_root)
	all_nodes.append(scene_root) # Include the root itself

	for node in all_nodes:
		if node is Node3D: # Optional: only select Node3D objects
...
func _input(event):
	if event is InputEventKey:
		if event.pressed and not event.echo and not event.meta_pressed: # Check meta modifier to avoid conflict with Mac command
			match event.keycode:
				KEY_A: #WORKING by forgeworksdev - Modified plugin
					...
					if focus != null:
						if focus.get_parent_control() != null:
							...
							if "Node3DEditorViewport" in focus.get_parent_control().name:
								_select_all_nodes()
								get_viewport().set_input_as_handled() #WORKING by forgeworksdev - Modified plugin
				KEY_Z:
					var focus = find_focused_control(get_tree().root)
